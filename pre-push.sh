...
if [ -n "$(pythonfiles)" ]; then
  ...
  if pythonfiles | xargs grep --color --with-filename -n "breakpoint()"; then
    echo "Error commiting changes: Please remove breakpoint and its usage"
    exit 1
  fi
fi
...
#     echo "You are about to commit on master. Running all tests first..."
...
#         echo "Some tests failed! Push broken code to master is not allowed! Aborting the push."
#         echo "Note: You can still push broken code on feature branches."
#         exit 1
#     fi
# fi
